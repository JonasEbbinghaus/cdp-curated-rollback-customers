# cdp_curated_rollback_customers
...
## Overview
- **Product name:** `cdp_curated_rollback_customers`
- **Layer:** `curated`
- *Domain:** `customers`

- **Owner / Team:** `{{TEAM_NAME}}` — contact: {{ EMAIL }}
...
The CLI that ran to create this repo performed the following actions for the given `cdp_curated_rollback_customers` and `curated`:
...
- Created an **Account level group** with read access for consumers: `cdp-curated-cdp_curated_rollback_customers-readers`. This group is intended to be the standard consumer group for granting read access to the product in UC.
...
- **`id`** *(Required)* — unique technical name of the data product (e.g., cdp_curated_rollback_customers). Was set automatically!
...
- **Access & Consumer Group** *(Required)* — group in the format cdp-curated-cdp_curated_rollback_customers-readers, description of how to request access.
...
- Group created: `cdp-curated-cdp_curated_rollback_customers-readers` — add consumers here to grant read access to UC objects for this product.
...
- [ ] Confirm that `cdp-curated-cdp_curated_rollback_customers-readers` group exists and members know how to request access
